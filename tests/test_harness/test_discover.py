...
def test_brainstorm_parses_candidates():
    llm = BrainstormLLM(
        '{"candidates": ['
        '{"id": "I-001", "title": "Add CLI", "description": "expose VALUE", "evidence": "REQ-1"},'
        '{"id": "I-002", "title": "Add caching", "description": "cache reads", "evidence": "code: slow reads"}'
        "]}"
    )
...
def test_brainstorm_handles_code_fences():
    llm = BrainstormLLM(
        ...
        '"description": "d", "evidence": "REQ-1"}]}\n```'
    )
    candidates = BrainstormStage(llm).run("g", "- REQ-1 pending", 1, None)
    assert len(candidates) == 1
    assert candidates[0].id == "I-1"


def test_brainstorm_rejects_candidate_without_evidence():
    llm = BrainstormLLM(
        '{"candidates": [{"id": "I-1", "title": "Speculative", '
        '"description": "unrelated scope"}]}'
    )
    assert BrainstormStage(llm).run("g", "", 1, None) == []


def test_brainstorm_rejects_unverifiable_evidence_text():
    llm = BrainstormLLM(
        '{"candidates": [{"id": "I-1", "title": "Speculative", '
        '"description": "scope", "evidence": "seems useful"}]}'
    )
    assert BrainstormStage(llm).run("g", "- REQ-1 passed", 1, None) == []
...
def test_brainstorm_tolerates_malformed_braced_json():
    llm = BrainstormLLM("prefix {not-json} suffix")
    assert BrainstormStage(llm).run("g", "", 1, None) == []
...
def test_prioritize_skips_integrated_fingerprints():
    ...
    first = ImprovementCandidate(id="I-0", title="Add CLI", description="d", score=0.95)
...
def test_brainstorm_prompt_includes_evidence_requirement():
    ...
    candidates = stage.run(
        ...
        1,
        None,
        code_signals="pytest: 2 failed",
        prior_titles=["Add CLI"],
    )
...
def test_prioritize_with_scores_classifies_and_caps():
    ...
    candidates = [
        ImprovementCandidate(
            id="I-1", title="High value thing", description="d", score=0.9
        ),
        ImprovementCandidate(
            id="I-2", title="Medium value thing", description="d", score=0.6
        ),
        ImprovementCandidate(
            id="I-3", title="Low value thing", description="d", score=0.3
        ),
    ]
...
def test_prioritize_with_scores_caps_overflow_to_parked():
    ...
    candidates = [
        ImprovementCandidate(
            id="I-1", title="High value alpha", description="d", score=0.95
        ),
        ImprovementCandidate(
            id="I-2", title="High value beta", description="d", score=0.90
        ),
    ]
    backlog, parked = stage.run(candidates, set(), use_scores=True)
    assert [c.id for c in backlog] == ["I-1"]
...
def test_prioritize_with_scores_parks_unscored_candidate():
    from onep.harness.discover import PrioritizeStage

    stage = PrioritizeStage(cap=3)
    candidate = ImprovementCandidate(
        id="I-1", title="Add CLI", description="d", score=None
    )
    backlog, parked = stage.run([candidate], set(), use_scores=True)
    assert backlog == []
    assert len(parked) == 1
    assert parked[0].score == DEFAULT_UNSCORED_SCORE
    assert parked[0].status == "parked"
